...
open class ImageViewerController: UIViewController {
    ...
    private lazy var btnClose: UIButton = {
        ...
        button.setImage(UIImage(named: "ic_close_viewer"), for: .normal)
        button.tintColor = .white
        button.addTarget(self, action: #selector(buttonCloseTap(sender:)), for: .touchUpInside)
        return button
    }()
    
    private lazy var btnShare: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage(named: "ic_share_viewer"), for: .normal)
        button.tintColor = .white
        button.addTarget(self, action: #selector(buttonShareTap(sender:)), for: .touchUpInside)
        return button
    }()
    ...
    fileprivate func setupControlsView() {
        view.insertSubview(btnClose, aboveSubview: scrollView)
        ...
        view.insertSubview(btnShare, aboveSubview: scrollView)
    ...
    @objc fileprivate func buttonCloseTap(sender: UIButton) {
        dismiss(animated: true, completion: dismissCompletion)
    }
    
    @objc fileprivate func buttonShareTap(sender: UIButton) {
    ...
    private func toggleControls(duration: TimeInterval = 0.1, delay: TimeInterval = 0) {
        let alpha: CGFloat = isControlsHidden ? 1.0 : 0.0
                
        UIView.animate(withDuration: duration, delay: delay, options: [], animations: {
            self.btnClose.alpha = alpha
            self.btnShare.alpha = alpha
        }) { [weak self] _ in
