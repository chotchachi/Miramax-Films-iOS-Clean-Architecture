...
public final class Defaults {
    ...
    /// Returns the value observable with the specified key.
    ///
    /// - Parameter key: The key.
    /// - Returns: A Observable of `ValueType` or nil if the key was not found.
    public func getObservable<ValueType>(for key: Key<ValueType>) -> Observable<ValueType?> {
        if isSwiftCodableType(ValueType.self) || isFoundationCodableType(ValueType.self) {
            return userDefaults.rx.observe(ValueType.self, key._key)
        }
        
        return userDefaults.rx.observe(Data.self, key._key)
            .map { data in
                if let data = data {
                    do {
                        let decoder = JSONDecoder()
